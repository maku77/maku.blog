...
- トレモで相手を CPU にして、**ずっとガードし続ける練習**（上下二択の見極め練習）。
- トレモで相手を CPU にして、**ダッシュガードでガードしまくる練習**。
  - 勘違いしてはダメだけど、ダッシュガードはガードされにいくのが目的ではなく、ライン上げを安全にするのが目的。
- トレモで相手を CPU にして、**起き上がり覚醒** の入力練習。一瞬白文字で REVERSAL と出れば成功。
- <strong>後ろ入れっぱダッシュガード</strong>（レバーレス限定）
  - 立ちガードで近づく場合：●◯◯ → ●◯● + dash → ●◯◯
  - しゃがみガードで近づく場合：●●◯ → ●◯● + dash → ●●◯
  - 後ろ入れっぱにするのは最速でガード状態に戻しやすいから（6 と dash を弾くだけでいい）。
  - 下の入力履歴のようにガードとガードの隙間が 1F に収まっていれば最速でダッシュガードできている。
    1F ダッシュは弾き入力が必要になるので現実的には難しいけど、**被弾率に直結するので 8F 以内には収めたい（立ちガードなら 4F 以内）**。
    {{< image w="200" src="img-dash-guard.jpg" >}}
  - ラムレザルには立ちダッシュガードで近づいてみる。
