...
- トレモで相手を CPU にして、**ずっとガードし続ける練習**。慣れたら **ダッシュガード** で割り込む。
- トレモで相手を CPU にして、**起き上がり覚醒** の入力練習。一瞬白文字で REVERSAL と出れば成功。
  入力ミスが多いときは仮想コントローラーで確認（トレーニング設定 → 画面表示設定 → 仮想コントローラーの表示）。

### 意識すること

- **根拠のある技を振る** ... 相手の動きの隙を探すつもりで。適当にボタンを押さない。相手の動きを予想する。ラム相手とかで遠距離で空振りしない。
- **防御ターンで暴れない** ... ゴールは「有利フレームまでガードしきる」ことや「距離をとる」こと。密接はファジージャンプ。やや密接は初段 FD。
- **中距離では対空を意識** ... 中距離で「空中ダッシュの光」が見えたら、{{< gg "6P" >}} を反射的に出す。微妙なときはガード。間違えてバックステップしない。
- **近距離での最速行動** ... 近距離の最速行動勝負では {{< gg "c.S" >}} を狙わず {{< gg "5P" >}} or {{< gg "2P" >}} で我慢。慣れたら {{< gg "5P" >}} 連打 → {{< gg "RCC" >}} へ繋ぐ。
- **差し返しは見た瞬間に** ... 中距離で相手の {{< gg "f.S" >}} などを差し返す場合、技の発生を見た瞬間にダッシュするつもりで。
