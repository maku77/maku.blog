---
title: "Cloudflare Pages で独自ドメインで Web サイト公開"
url: "p/efcf8xn/"
date: "2026-05-08"
tags: ["cloudflare", "dns"]
---
...
ここでは、**`example.com`** という独自ドメインを取得したものとして説明します。

{{% note %}}
実はドメイン管理（レジストラ）も Cloudflare Registrar を使った方がお財布に優しく（純粋にレジストリやICANNからの請求額しかかからない）、設定画面もわかりやすいらしいので、あとでレジストラを移管するつもりです。
確かにお名前.com は「サービス維持調整費」みたいな値上げメールがよく来てどんどん更新料金が上がってる気がします。
ドメイン取得時だけ割引しておいて更新料金は高い、というモデルもいまいちですね。
{{% /note %}}

ちなみに、ネームサーバーが正しく設定されていない状態だと、下記のように `nslookup` でのドメイン名の解決結果が `SERVFAIL` エラーになります。
ドメイン自体が存在しない場合は `NXDOMAIN` エラーになります。
ネームサーバーが正しく設定されているけど、A レコードなどの情報がない場合は、`No answer` 表示になります。
...
上記の例では、すでにレジストラ側に別のネームサーバー (`*.dns.ne.jp`) が設定されているよ、と表示されていますが、これを Cloudflare のネームサーバーのアドレスで上書きしてやります。
...
`/public` ディレクトリ以下のコンテンツをトップページとして公開することとします。
